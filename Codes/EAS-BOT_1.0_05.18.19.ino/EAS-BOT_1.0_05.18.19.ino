...
/*>>>>>>>>>>>>>>>Fingers<<<<<<<<<<<<<<<*/
    ...
    if (realservo >= 21000 && realservo < 21180) {
      int righthand = realservo;
      righthand = map(righthand, 21000, 21180, 0, 180);
      RTF, RIF, RMF, RRF, RPF.write(righthand);
      delay(10);
    }
    if (realservo >= 22000 && realservo < 22180) {
      int lefthand = realservo;
      lefthand = map(lefthand, 22000, 22180, 0, 180);
      LTF, LIF, LMF, LRF, LPF.write(lefthand);
      delay(10);
    }
   }
/*>>>>>>>>>>>>>>>Sensors<<<<<<<<<<<<<<<*/
